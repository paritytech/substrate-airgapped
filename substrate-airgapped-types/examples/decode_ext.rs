use codec::Decode;
use substrate_airgapped_types::{frame::balances::TransferArgs, PolkadotRuntime, UncheckedExtrinsic};

fn main() -> () {
	let balances_transfer_alice = b"";

	type Ext = UncheckedExtrinsic<PolkadotRuntime, TransferArgs<PolkadotRuntime>>;
	let result: Ext = Decode::decode(&mut &balances_transfer_alice[..]).unwrap();

	println!("Decoded ext: {:#?}", result);

	()
}
